...
class Model(nn.Module):
    ...
    def get_outputs_for_camera_with_confidence(self, camera: Cameras, method: str, confidence_cache: dict[str, torch.Tensor], output_path: Path, obb_box: Optional[OrientedBox] = None) -> Dict[str, torch.Tensor]:
        """Models which support confidence should override this

        Confidence map should be in outputs["confidence"]
        """
        return self.get_outputs_for_camera(camera, obb_box)

    def precompute_confidence(self, method: str, output_path: Path, train_loader: FixedIndicesEvalDataloader):
        """Called with all training data before rendering
        """
        return {}
