...
class ColmapDataParser(DataParser):
    ...
    def _get_all_images_and_cameras(self, recon_dir: Path, split):
        ...
        for im_id in ordered_im_id:
            ...
            if self.config.depths_path is not None:
                frame["depth_path"] = (
                    (self.config.data / self.config.depths_path / (im_data.name.split('.')[0]+'_pred')).with_suffix(".npy").as_posix()
                )
    ...
    def _get_image_indices(self, image_filenames, split):
        ...
        # if (self.config.data / f"{split}_list.txt").exists():
            ...
            if self.config.eval_mode == "fraction":
                ...
                print('______________ use filename ______________')
                i_train, i_eval = get_train_eval_split_filename(image_filenames, str(self.config.data))
        ...
        self.i_train, self.i_eval = i_train, i_eval
    ...
    def _generate_dataparser_outputs(self, split: str = "train", **kwargs):
        ...
        print('colmap_path',colmap_path)
        
        meta = self._get_all_images_and_cameras(colmap_path, split)
        camera_type = CAMERA_MODEL_TO_TYPE[meta["camera_model"]]

        image_filenames = [Path(frame["file_path"]) for frame in meta["frames"]]
        ...
        print('Downscale factor', self._downscale_factor)
    ...
    def _load_3D_points(self, colmap_path: Path, transform_matrix: torch.Tensor, scale_factor: float):
        ...
        print('Total points shape ', points3D.shape)
