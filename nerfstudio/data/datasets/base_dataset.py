...
class InputDataset(Dataset):
    ...
    def get_data(self, image_idx: int, image_type: Literal["uint8", "float32"] = "float32") -> Dict:
        ...
        data = {"image_idx": image_idx, "image": image, "image_name": str(self._dataparser_outputs.image_filenames[image_idx])}
        ...
        if self._dataparser_outputs.metadata['depth_filenames'] is not None:
            ...
            data["depth_image"] = get_depth_image_from_path(filepath=depth_filepath, height=data["image"].shape[0], width=data["image"].shape[1] , scale_factor=self.scale_factor)
            assert (
                data["depth_image"].shape[:2] == data["image"].shape[:2]
            ), f"Mask and image have different shapes. Got {data['depth_image'].shape[:2]} and {data['image'].shape[:2]}"
