...
class FullImageDatamanager(DataManager, Generic[TDataset]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.test_dataparser_outputs: DataparserOutputs = self.dataparser.get_dataparser_outputs(split="test")
        self.image_names = self.test_dataparser_outputs.image_filenames
    ...
    def _load_images(
        ...
        def undistort_idx(idx: int) -> Dict[str, torch.Tensor]:
            ...
            K, image, mask, depth = _undistort_image(camera, distortion_params, data, image, K)
    ...
    def fixed_indices_eval_dataloader(self) -> List[Tuple[Cameras, Dict]]:
        ...
        _data = deepcopy(self.cached_eval)
        _cameras = deepcopy(self.eval_dataset.cameras).to(self.device)
        cameras,data = [],[]
        for i in image_indices:
            # data[i]["image"] = data[i]["image"].to(self.device)
            data.append({"image":_data[i]["image"].to(self.device),"image_name":str(self.image_names[i])})
            ...
            cam.metadata["cam_name"] = self.test_dataparser_outputs.image_filenames[i]
            cameras.append(cam)
    ...
    def next_train(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        camera.metadata["cam_name"] = self.train_dataparser_outputs.image_filenames[image_idx]
    ...
    def next_eval(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        camera.metadata["cam_name"] = self.test_dataparser_outputs.image_filenames[image_idx]
...
def _undistort_image(
    ...
    if camera.camera_type.item() == CameraType.PERSPECTIVE.value:
        ...
        if np.any(distortion_params):
            print('undistorting!!!', data['image_name'])
            newK, roi = cv2.getOptimalNewCameraMatrix(K, distortion_params, (image.shape[1], image.shape[0]), 0)
            image = cv2.undistort(image, K, distortion_params, None, newK)  # type: ignore
        else:
            print('no distortion!!!', data['image_name'])
            newK = K
