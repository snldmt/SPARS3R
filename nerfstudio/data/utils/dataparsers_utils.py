...
def get_train_eval_split_filename(image_filenames: List, datapath=None) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    train_txt = os.path.join(datapath, "train.txt")
    if not os.path.exists(train_txt):
        train_txt = os.path.join(datapath, "train_list.txt")
    test_txt = os.path.join(datapath, "test.txt")
    if not os.path.exists(test_txt):
        test_txt = os.path.join(datapath, "test_list.txt") # type: ignore

    if os.path.exists(train_txt) and os.path.exists(test_txt):
        print("---------------------------- Using train.txt and test.txt to split train/eval")
        with open(train_txt) as f:
            train_files = f.readlines()
            train_files = [e.strip() for e in train_files]
        with open(test_txt) as f:
            test_files = f.readlines()
            test_files = [e.strip() for e in test_files]
        
        for idx, basename in zip(i_all, basenames):
            if basename in train_files:
                i_train.append(idx)
            elif basename in test_files:
                i_eval.append(idx)
            else:
                raise ValueError("frame should be in train.txt or test.txt to use this eval-frame-filename eval mode")
    else:
        for idx, basename in zip(i_all, basenames):
