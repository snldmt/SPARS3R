...
class Optimizers:
    ...
    def load_optimizers(self, loaded_state: Dict[str, Any], test_opt_pose=False) -> None:
        ...
        """
        if test_opt_pose:
            print('inside load state dict for optimizers, now only optimizing test pose, so freezing the rest')
            for k, v in loaded_state.items():
                try:
                    ...
                    from nerfstudio.utils.rich_utils import CONSOLE
                    CONSOLE.print(
                        f"optimizers does not have this key {k} \n",
                        style="bold yellow",
                    )
        else:
            for k, v in loaded_state.items():
    ...
    def load_schedulers(self, loaded_state: Dict[str, Any], test_opt_pose=False) -> None:
        ...
        """
        if test_opt_pose:
            print('inside load state dict for schedulers, now only optimizing test pose, so freezing the rest')
            for k, v in loaded_state.items():
                try:
                    ...
                    from nerfstudio.utils.rich_utils import CONSOLE
                    CONSOLE.print(
                        f"schedulers does not have this key {k} \n",
                        style="bold yellow",
                    )
