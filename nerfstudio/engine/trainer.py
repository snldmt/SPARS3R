...
class Trainer:
    ...
    def _load_checkpoint(self) -> None:
        ...
        if load_dir is not None:
            ...
            self._start_step = 0 if self.pipeline.model.config.test_opt_pose else loaded_state["step"] + 1
            # self._start_step = 0

            # load the checkpoints for pipeline, optimizers, and gradient scalar
            self.pipeline.load_pipeline(loaded_state["pipeline"], loaded_state["step"])

            self.optimizers.load_optimizers(loaded_state["optimizers"], test_opt_pose=self.pipeline.model.config.test_opt_pose)

            if "schedulers" in loaded_state and self.config.load_scheduler:
                self.optimizers.load_schedulers(loaded_state["schedulers"], test_opt_pose=self.pipeline.model.config.test_opt_pose)
