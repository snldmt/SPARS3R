...
def get_points(outputs, pipeline, ray_bundle,
               rgb_output_name, depth_output_name,
               use_bounding_box, crop_obb, bounding_box_min, bounding_box_max,):
    if rgb_output_name not in outputs:
        CONSOLE.rule("Error", style="red")
        CONSOLE.print(f"Could not find {rgb_output_name} in the model outputs", justify="center")
        CONSOLE.print(f"Please set --rgb_output_name to one of: {outputs.keys()}", justify="center")
    ...
    if isinstance(depth_output_name, str):
        depth = outputs[depth_output_name]    
    else:
        depth = depth_output_name
    ...
    return point, rgb, view_direction
