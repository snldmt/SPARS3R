...
def eval_load_checkpoint(config: TrainerConfig, pipeline: Pipeline) -> Tuple[Path, int]:
    ...
    pipeline.load_pipeline(loaded_state["pipeline"], loaded_state["step"])#, loaded_state['cams_embedding'])
...
def eval_setup(
    ...
    if eval_train:
        config.pipeline.datamanager.dataparser.eval_train = True

    
    config.pipeline.datamanager.dataparser.dsim_loss_extrapolate = False


    # load checkpoints from wherever they were saved
    config.load_dir = config.get_checkpoint_dir()

    ### YT: added this to allow for loading a specific checkpoint
