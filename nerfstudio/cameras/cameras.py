...
class Cameras(TensorDataclass):
    ...
    def rescale_output_resolution(
        ...
        if type(scaling_factor) == float or type(scaling_factor) == int:
            scale_w, scale_h = scaling_factor, scaling_factor
        else:
            scale_w, scale_h = scaling_factor[...,[0]], scaling_factor[...,[1]]
            # print(scale_w.shape, scale_h.shape, self.height.shape, self.width.shape,'%%%%%%%%%')
        self.fx = self.fx * scale_w
        self.fy = self.fy * scale_h
        self.cx = self.cx * scale_w
        self.cy = self.cy * scale_h
        self.height = torch.round(self.height * scale_h).to(torch.int64)
        self.width = torch.round(self.width * scale_w).to(torch.int64)
        # print(self.height, self.width, scale_w, scale_h)